function [ descrs ] = encodeImageTexton( encoder, im, varargin )
% ENCODEIMAGE   Apply Texton encoder to an image
...
opts.useMasks = false;
opts.ntex =3; % TODO fix this  pass as a paramter
opts.featureDir = 'C:\DTD\data\dtd\features';
...
for c = 1:numChunks
  ...
  if ~isempty(opts.cacheDir) && exist(chunkPath)
    ...
    data = processChunkTexton(encoder, im(range), opts.ntex) ;
...
% --------------------------------------------------------------------
function psi = processChunkTexton(encoder, im, ntex)
% --------------------------------------------------------------------
v = ver;
...
if numel(im) > 1 && hasParallelToolbox
  parfor i = 1:numel(im)
    psi{i} = encodeOneTexton(encoder, im{i}, ntex) ;
  end
else
...
% --------------------------------------------------------------------
function psi = encodeOneTexton(encoder, imName, ntex)
% --------------------------------------------------------------------
...
fileName1 = strrep(imName, 'jpg', 'mat');
fileName = strrep(fileName1,'images','features');

if  exist(fileName,'file')
    fprintf('%s: loading filtered image from %s\n', fileName, fileName) ;
    load(fileName, 'fim') ;
else
      ima = encoder.readImageFn(imName, encoder.scaling) ;
      fim = encoder.extractorFn(ima) ;
      %fim = GetFilterFeatures(ima);
      save(fileName, 'fim') ;
end



%TODO temp hack to get through the code
psi = {} ;
psi = cat(1, fim{1}(1:10,:) ) ;
%features = fim;
