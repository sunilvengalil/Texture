...
legendtype = ['r+';'g+';'b+';'k+';'r*';'g*';'b*';'k*'];
legendtypestr = cellstr(legend);
...
%plot
 ...
 for orient = 1 : numOrient
 strcat(legendstr,num2str(orient))
 end
 ...
 plot(sigmas,maxFilterResponse(1,:,1),'r');
 hold on 
 plot(sigmas,maxFilterResponse(2,:,1),'g');
 hold on;
 plot(sigmas,maxFilterResponse(3,:,1),'b');
 hold on 
 plot(sigmas,maxFilterResponse(4,:,1),'k');
